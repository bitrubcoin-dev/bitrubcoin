...
*Setup instructions for a Gitian build of BitRubCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitRubCoin
...
- [Building BitRubCoin](#building-bitrubcoin-core)
...
Clone the git repositories for bitrubcoin and Gitian.
...
in the bitrubcoin repository under 'Fetch and create inputs' to install sources which require
...
To build BitRubCoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitrubcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitrubcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/BitRubCoin/bitrubcoin
...
For example:
```bash
URL=https://github.com/thrasher-/bitrubcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitrubcoin=${COMMIT} --url bitrubcoin=${URL} ../bitrubcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitrubcoin=${COMMIT} --url bitrubcoin=${URL} ../bitrubcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitrubcoin=${COMMIT} --url bitrubcoin=${URL} ../bitrubcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitrubcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitrubcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitrubcoin
SIGPATH=/some/root/path/bitrubcoin-detached-sigs

./bin/gbuild --url bitrubcoin=${BTCPATH},signature=${SIGPATH} ../bitrubcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitrubcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitrubcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitrubcoin-osx-build.assert
```
...
[bitrubcoin/gitian.sigs.ltc](https://github.com/BitRubCoin/gitian.sigs.ltc/) repository, or if that's not possible create a pull
